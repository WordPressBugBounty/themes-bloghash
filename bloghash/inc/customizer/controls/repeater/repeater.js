...
function BloghashLinkField($el) {
    ...
    const open = () => {
        ...
        var $textarea = jQuery('<textarea id="bloghash-link-textarea" style="display:none;"></textarea>');
        if(!jQuery('#bloghash-link-textarea').length)jQuery('body').append($textarea);

        var val = getNodeValue();
        wpLink.open('bloghash-link-textarea', val.url, val.title, null);
    }
    ...
    const  onClose = () => {
        ...
        jQuery('#bloghash-link-textarea').remove();
      }
...
( function( api, $ ) {
    api.controlConstructor['bloghash-repeater'] = api.Control.extend({
        ready: function() {
            var control = this;
            setTimeout( function() {
                control._init();
        ...
        _init: function() {
            ...
            control.autocorrect_range_input_number = function( input_number, timeout ) {
                ...
                var range_input = input_number,
                    ...
                    reset = parseFloat( range.find( '.bloghash-reset-range' ).attr( 'data-reset_value' ) ),
            ...
            if ( ! window._upload_bloghash ) {
                ...
                window._upload_bloghash = wp.media({
            ...
            window._upload_bloghash.on( 'close', function() {

                // get selections and save to hidden input plus other AJAX stuff etc.
                var selection = window._upload_bloghash.state().get( 'selection' );

            
            });
            window.media_current = {};
            window.media_btn = {};
            window._upload_bloghash.on( 'select', function() {

                // Grab our attachment selection and construct a JSON representation of the model.
                var media_attachment = window._upload_bloghash.state().get( 'selection' ).first().toJSON();
            ...
            control.handleGradient = function( $context ) {
                var control = this,
                    ...
                    range_input,
                    value,
                    bloghash_range_input_number_timeout,
                ...
                $( '.item-gradient', $context ).each( function() {
                    ...
                    _item.find( '.bloghash-range-wrapper' ).each( function() {
                        var $this = $( this );
                        ...
                        $this.find( 'input.bloghash-range-input' ).on( 'change keyup', function() {
                            control.autocorrect_range_input_number( $( this ), 1000, bloghash_range_input_number_timeout );
                        }).on( 'focusout', function() {
                            control.autocorrect_range_input_number( $( this ), 0, bloghash_range_input_number_timeout );
                        });
                    ...
                    // Visibility deps.
                    _item.on( 'change', '[data-option="background-type"], [data-option="gradient-type"]', function() {
                    ...
                    _item.on( 'change', '.bloghash-select-wrapper', function() {
                        control.update_value();
                    });
            ...
            control.handleBackground = function( $context ) {
                ...
                $( '.item-design-options', $context ).each( function() {
                    ...
                    _item.on( 'click', '.background-image-upload-remove-button', function( e ) {
                    ...
                    // Linked button
                    _item.on( 'click', '.bloghash-spacing-linked', function() {
                    ...
                    // Unlinked button
                    _item.on( 'click', '.bloghash-spacing-unlinked', function() {
					...
					$.each(
						...
						function ( fieldId, field ) {
							if ( field.type === 'select' && field.is_select2 ) {
								...
								if ( $select.length ) {
									var select2Config = {
										placeholder: field.placeholder || ( typeof bloghash_customizer_localized !== 'undefined' ? bloghash_customizer_localized.strings.select_category : 'Select' ),
										allowClear: true,
										minimumInputLength: 0,
										width: '100%'
									};

									var ajaxUrl = ( typeof bloghash_customizer_localized !== 'undefined' && bloghash_customizer_localized.ajaxurl ) ? bloghash_customizer_localized.ajaxurl : ajaxurl;
									var nonce = control.params.nonce || ( typeof bloghash_customizer_localized !== 'undefined' ? bloghash_customizer_localized.wpnonce : '' );
									...
									if ( field.data_source ) {
										select2Config.ajax = {
											...
											data: function( params ) {
												return {
													action: 'bloghash_load_select2_data',
													search: params.term || '',
													page: params.page || 1,
											...
											error: function(error) {
												console.error( typeof bloghash_customizer_localized !== 'undefined' ? bloghash_customizer_localized.strings.error_loading_data : 'Error loading data', error );
											}
										};
									}
            ...
            control.linkPicker = function( $context ){
                if(item_link = $context.find('.bloghash-field-link')) {
                  const a =  new BloghashLinkField(item_link);
                  a.init();
                }
            }
            ...
            control.actions = function( $context ) {
                if ( control.params.live_title_id ) {
                    ...
                    if ( 'bloghash_section_order_styling' === control.id ) {
...
jQuery( document ).ready( function( $ ) {
    ...
    var options_font_type = '',
        icon_group = '';
    $.each( Bloghash_Icon_Picker.fonts, function( key, font ) {
        font = $.extend({}, {
            url: '',
            name: '',
            prefix: '',
    ...
    icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Bloghash_Icon_Picker.search );
