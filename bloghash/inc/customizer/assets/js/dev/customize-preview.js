...
(function ($) {
	...
	if (api.selectiveRefresh) {
		...
		api.selectiveRefresh.bind('partial-content-rendered', function (placement) {
			...
			// Hero Slider.
			if ('bloghash_hero_slider_post_number' === placement.partial.id || 'bloghash_hero_slider_elements' === placement.partial.id || 'bloghash_hero_type' === placement.partial.id) {
				document.querySelectorAll(placement.partial.params.selector).forEach((item) => {
					bloghashHeroSlider(item);
					document.querySelectorAll('#' + item.id + ' .bloghash-swiper').forEach(bloghashSliderInit);
				});
			}

			// PYML Post.
			if ('bloghash_pyml_post_number' === placement.partial.id || 'bloghash_pyml_elements' === placement.partial.id) {
				document.querySelectorAll(placement.partial.params.selector).forEach((item) => {
					document.querySelectorAll('#' + item.id + ' .bloghash-swiper').forEach(bloghashSliderInit);
				});
			}
