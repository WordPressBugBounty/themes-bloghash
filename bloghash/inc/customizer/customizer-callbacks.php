 ...
 */
function bloghash_sanitize_number( $val ) {
 ...
 */
function bloghash_sanitize_toggle( $input, $setting ) {

	if ( 'bloghash_enable_front_page' === $setting ) {
		sanitize_key( $input );
	}
 ...
 * @return boolean
 */
function bloghash_sanitize_checkbox( $input ) {
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_select( $input, $setting ) {
	...
	if ( $control && ! empty( $control->is_select2 ) && ! empty( $control->data_source ) ) {
		$dynamic = bloghash_sanitize_select_dynamic_source( $input, $setting, $control );
		if ( false !== $dynamic ) {
			return $dynamic;
		}
	}
 ...
 */
function bloghash_sanitize_select_dynamic_source( $input, $setting, $control ) {
	...
	if ( $multiple ) {
		...
		$valid_ids = bloghash_sanitize_select2_valid_ids( $input, $data_source, $data_source_name );
		if ( empty( $valid_ids ) ) {
			return array();
		}
	...
	$valid_ids = bloghash_sanitize_select2_valid_ids( array( $input ), $data_source, $data_source_name );
	$valid_ids = array_map( 'strval', $valid_ids );
 ...
 */
function bloghash_sanitize_select2_valid_ids( $input_ids, $data_source, $data_source_name = null ) {
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_textarea( $input, $setting ) {
 ...
 */
function bloghash_sanitize_color( $color ) {
	...
	if ( false === strpos( $color, 'rgba' ) ) {
		return bloghash_sanitize_hex_color( $color );
	}

	return bloghash_sanitize_alpha_color( $color );
}
 ...
 */
function bloghash_sanitize_hex_color( $color ) {
 ...
 */
function bloghash_sanitize_alpha_color( $color ) {
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_image( $image, $setting ) {
	/*
 ...
 */
function bloghash_sanitize_widget( $widgets, $setting ) {
 ...
 */
function bloghash_no_sanitize( $val ) {
	return $val;
}
 ...
 * @return string
 */
function bloghash_sanitize_order( $input ) {
	...
	$filter_empty = array_filter( $json, 'bloghash_not_empty' );
 ...
 * @return bool
 */
function bloghash_not_empty( $val ) {
	return ! empty( $val );
}
 ...
 */
function bloghash_sanitize_background( $background ) {
 ...
 */
function bloghash_sanitize_spacing( $values, $setting, $default = array() ) {
 ...
 * @param mixed  $default Default Value.
 */
function bloghash_sanitize_range( $value, $setting, $default = '' ) {
 ...
 * @param object $setting WP Customize Setting instance.
 */
function bloghash_sanitize_responsive( $values, $setting ) {

	$control       = $setting->manager->get_control( $setting->id );
	$control_type  = str_replace( 'bloghash-', '', $control->type );
	...
	if ( is_array( $control->responsive ) && ! empty( $control->responsive ) ) {
		...
		foreach ( $values as $device => $value ) {
			...
			$values[ $device ] = call_user_func_array(
				'bloghash_sanitize_' . $control_type,
				array(
					$values[ $device ],
					$setting,
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_typography( $value, $setting ) {
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_design_options( $value, $setting ) {
	...
	if ( isset( $control->display ) ) {
		...
		if ( isset( $control->display['background'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['background'] ) ) {
				...
				if ( isset( $value['background-color'] ) ) {
					$sanitized['background-color'] = bloghash_sanitize_color( $value['background-color'] );
				}
			}
			...
			if ( array_key_exists( 'gradient', $control->display['background'] ) ) {
				...
				if ( isset( $value['gradient-color-1'] ) ) {
					$sanitized['gradient-color-1'] = bloghash_sanitize_color( $value['gradient-color-1'] );
				}

				if ( isset( $value['gradient-color-2'] ) ) {
					$sanitized['gradient-color-2'] = bloghash_sanitize_color( $value['gradient-color-2'] );
				}

				if ( isset( $value['gradient-color-1-location'] ) ) {
					$sanitized['gradient-color-1-location'] = bloghash_sanitize_range( $value['gradient-color-1-location'], $setting );
				}

				if ( isset( $value['gradient-color-2-location'] ) ) {
					$sanitized['gradient-color-2-location'] = bloghash_sanitize_range( $value['gradient-color-2-location'], $setting );
				}
				...
				if ( isset( $value['gradient-linear-angle'] ) ) {
					$sanitized['gradient-linear-angle'] = bloghash_sanitize_range( $value['gradient-linear-angle'], $setting );
				}
		...
		if ( isset( $control->display['color'] ) ) {
			foreach ( $control->display['color'] as $id => $title ) {
				if ( isset( $value[ $id ] ) ) {
					$sanitized[ $id ] = bloghash_sanitize_color( $value[ $id ] );
				}
			}
		}
		...
		if ( isset( $control->display['border'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['border'] ) && isset( $value['border-color'] ) ) {
				$sanitized['border-color'] = bloghash_sanitize_color( $value['border-color'] );
			}
			...
			if ( isset( $value['separator-color'] ) ) {
				$sanitized['separator-color'] = bloghash_sanitize_color( $value['separator-color'] );
			}
 ...
 * @param object $setting  Setting object.
 */
function bloghash_sanitize_sortable( $value, $setting ) {
 ...
 */
function bloghash_repeater_sanitize( $input, $setting ) {
	$control = $setting->manager->get_control( $setting->id );
			...
			if ( isset( $fields[ $id ] ) ) {
				switch ( strtolower( $fields[ $id ]['type'] ) ) {
					...
					case 'editor':
						...
						break;
					case 'color':
						$data[ $i ][ $id ] = bloghash_sanitize_hex_color( $value );
						break;
					case 'coloralpha':
						$data[ $i ][ $id ] = bloghash_sanitize_alpha_color( $value );
						break;
					case 'checkbox':
						$data[ $i ][ $id ] = bloghash_sanitize_checkbox( $value );
						break;
					case 'select':
						$data[ $i ][ $id ] = '';
					...
					case 'gradient':
						...
						break;
					case 'design-options':
						bloghash_sanitize_design_options( $value, $setting );
						break;
					default:
						$data[ $i ][ $id ] = wp_kses_post( $value );
				}
